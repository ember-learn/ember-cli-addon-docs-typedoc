...
module.exports = class Function extends Entity {
  ...
  extractSignature(signature) {
    ...
    let typeParams = (signature.typeParameters || []).map((typeParam) => {
      let constraint = typeParam.type ? ` extends ${this.typeToString(typeParam.type)}` : '';
  ...
  typeToString(type) {
    ...
    if (stringified === 'function') {
      let { params, typeParams, returns } = this.extractSignature(type.declaration.signatures[0]);
      stringified = `(${params.map(p => `${p.name}: ${p.type}`).join(', ')}) => ${returns.type}`;
