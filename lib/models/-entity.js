...
module.exports = class Entity {
  ...
  constructor(parent, ref, options) {
    ...
    this.tags = ((ref.comment && ref.comment.tags) || []).map((tag) =>
      this.extractTag(tag)
    );

    if (ref.decorators) {
      this.decorators = ref.decorators.map((decorator) =>
        this.extractDecorator(decorator)
      );
    }
  ...
  extractFile(ref, { importPathForFile }) {
    return importPathForFile(ref.sources[0].file.fullFileName);
  }
