...
launch(new class TypedocWorker {
  constructor() {
  ...
  emit(outFile) {
    ...
    let context = new Context(converter, [], program.getTypeChecker(), program);
    ...
    let json = Serializer.serialize('module', docs.modules.filter(mod => {
      return mod.file && this.packages.some(pkg => mod.file === pkg || mod.file.startsWith(`${pkg}/`));
    }));
  ...
  for (let logicalPath of Object.keys(pathsHash)) {
    ...
    for (let relativePhysicalPath of physicalPaths) {
      if (relativePhysicalPath.endsWith('/*') !== isGlob) {
        throw new Error(`Mismatched path specifier; logical: ${logicalPath}, physical: ${relativePhysicalPath}`);
      }
  ...
  return (filePath) => {
    ...
    do {
      ...
      if (candidate in mappings) {
        return mappings[candidate].replace(/\*$/, fullPath.substring(candidate.length - 1));
      }
